...
from src.agents.dqn.utils import ReplayBuffer, Logger, TestMetric, set_global_seed, PPOBuffer, ReplayBufferPPO
...
class PPO:
    ...
    def __init__(
        ...
        for n_spins in set([env.action_space.n for env in self.envs]):
            #self.replay_buffers[n_spins] = PPOBuffer()
            self.replay_buffers[n_spins] = ReplayBufferPPO(self.replay_buffer_size)
        ...
        self.value_losses_save_path = os.path.join(os.path.split(self.test_save_path)[0], "value_losses.pkl")
        self.entropy_save_path = os.path.join(os.path.split(self.test_save_path)[0], "entropy.pkl")
        self.state_values_save_path = os.path.join(os.path.split(self.test_save_path)[0], "state_values.pkl")
    ...
    def learn(self, timesteps, verbose=False):
        ...
        for timestep in range(timesteps):
            ...
            #is_training_ready=True
            # Choose action
            action, output = self.act(state.to(self.device).float(), is_training_ready=is_training_ready)
            
            #here insert distribution find
            ...
            #('state', 'action', 'reward', 'state_next', 'done', 'logprobs', 'values')
            self.replay_buffer.add(state, action, reward, state_next, done, output['logprob'], output['values'])
            ...
            if is_training_ready:
                ...
                if timestep % self.update_frequency == 0:
                    ...
                    #transitions = self.replay_buffer.get_list(self.device)#
                    transitions = self.get_random_replay_buffer().sample(self.minibatch_size, self.device)

                    # Train on selected batch
                    loss, policy_loss, value_loss, entropy, state_value = self.train_step(transitions)
                    losses.append([timestep,loss])
                    policy_losses.append([timestep, policy_loss])
                    value_losses.append([timestep, value_loss])
                    entropys.append([timestep, entropy])
                    state_values.append([timestep, state_value])
        ...
        with open(self.policy_losses_save_path, 'wb+') as output:
            ...
            if verbose:
                print('policy losses saved to {}'.format(self.policy_losses_save_path))
               
        with open(self.value_losses_save_path, 'wb+') as output:
            pickle.dump(np.array(value_losses), output, pickle.HIGHEST_PROTOCOL)
            if verbose:
                print('value losses saved to {}'.format(self.value_losses_save_path))
                
        with open(self.entropy_save_path, 'wb+') as output:
            pickle.dump(np.array(entropys), output, pickle.HIGHEST_PROTOCOL)
            if verbose:
                print('entropy saved to {}'.format(self.entropy_save_path))
                
                
        with open(self.state_values_save_path, 'wb+') as output:
            pickle.dump(np.array(state_values), output, pickle.HIGHEST_PROTOCOL)
            if verbose:
                print('state_values saved to {}'.format(self.state_values_save_path))
    ...
    def train_step(self, transitions):
        ...
        states, actions, rewards, states_next, dones, logprobs, values = transitions
        #process rewards
        #
         
        advantages = self.compute_advantages(rewards, values, dones)#rewards - new_value
        advantages = torch.tensor(self.reward_discount(advantages, dones, self.gamma), device = self.device)
        if advantages.shape[-1] > 1:
            advantages = self.reward_norm(advantages)
        #detach adavantage
        advantages.detach_()
        rewards = torch.tensor(self.reward_discount(rewards, dones, self.gamma), device = self.device)
        if rewards.shape[-1] > 1:
            rewards = self.reward_norm(rewards)
        #evaluation
        for i in range(4):
            new_d_inputs, new_value = self.network(states_next.float())
            new_value = new_value.squeeze()
            new_distribution = torch.distributions.Categorical(logits = new_d_inputs)
            ...
            if self.eps_clip is not None:
                ratio_clipped = torch.clamp(ratios, 1-self.eps_clip, 1+self.eps_clip)
                surr_clipped = ratio_clipped*advantages
                policy_loss = -torch.min(surr, surr_clipped) - 0.01*entropy
            else:
                policy_loss = surr - 0.01*entropy

            value_loss = 0.5*torch.pow(new_value - rewards.squeeze(), 2) #pow
            if self.eps_clip is not None:
                values_clipped = values + torch.clamp(new_value - values, -self.eps_clip, self.eps_clip)
                value_loss_clipped = torch.pow(values_clipped - rewards.squeeze(), 2)
                value_loss = torch.max(value_loss, value_loss_clipped)#min
            policy_loss = policy_loss.mean()
            value_loss = value_loss.mean()
            final_loss = policy_loss + value_loss
            state_values = new_value.mean()


    #             policy_losses.append(policy_loss)
    #             value_losses.append(value_loss)
    #             final_losses.append(final_loss)
    #             state_values_l.append(state_values)
...
#             with torch.no_grad():
#                 if self.double_dqn:
#                     greedy_actions = self.network(states_next.float())[0].argmax(0, True)
#                     q_value_target = self.target_network(states_next.float())[0].gather(0, greedy_actions)
#                 else:
#                     q_value_target = self.target_network(states_next.float())[0].max(0, True)[0]

#         else:
#             target_preds,_ = self.target_network(states_next.float())
...
#             with torch.no_grad():
#                 if self.double_dqn:
#                     network_preds,_ = self.network(states_next.float())
...
#         # Calculate Q value
#         q_value = self.network(states.float())[0].gather(0, actions)
...
#         loss = self.loss(q_value, td_target, reduction='mean')
        ...
        return final_loss.item(), policy_loss.item(), value_loss.item(), entropy.mean().item(), state_values.item()
        #return np.mean(final_losses), np.mean(policy_losses), np.mean(value_losses), entropy.mean().item(), np.mean(state_values_l)
    def compute_advantages(self, rewards, values, dones):
        td_errors = rewards + self.gamma * values * (1. - dones) - values#[:, :, :-1]
...
#         d_inputs, value = self.network(states)

#         if acting_in_reversible_spin_env:
#             distribution = torch.distributions.Categorical(logits = d_inputs)
#             actions = distribution.sample()
#             logprob = distribution.log_prob(actions)
#             return {"distribution": distribution, "values": value, "actions":actions, "logprob":logprob}
#         else:
#             #TO CHANGE LATER
#             if d_inputs.dim() == 1:
#                 x = (states[0, :] == self.allowed_action_state)#.nonzero()
#                 #actions = x[d_inputs[x].argmax().item()]#.item()
#                 d_inputs = d_inputs.masked_fill(x, 0)
#             else:
#                 disallowed_actions_mask = (states[:, :, 0] != self.allowed_action_state)
#                 d_inputs = d_inputs.masked_fill(disallowed_actions_mask, 0)
#                 #.cpu().numpy()
#             distribution = torch.distributions.Categorical(logits = d_inputs)
#             actions = distribution.sample()
#             logprob = distribution.log_prob(actions)
#             return {"distribution": distribution, "values": value, "actions":actions, "logprob":logprob}
    ...
    def predict(self, states, acting_in_reversible_spin_env=None):
        ...
        d_inputs, value = self.target_network(states)
        distribution = torch.distributions.Categorical(logits = d_inputs)
        if acting_in_reversible_spin_env:
            ...
            if d_inputs.dim() == 1:
                ...
                d_inputs_allowed = d_inputs.masked_fill(disallowed_actions_mask, -10000)
    ...
    def evaluate_agent(self, batch_size=None):
        ...
        while i_comp < self.test_episodes:
            ...
            output = self.predict(torch.FloatTensor(np.array(obs_batch)).to(self.device),

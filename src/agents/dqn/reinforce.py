...
#                     is_training_ready=True
            ...
            #('state', 'action', 'reward', 'state_next', 'done', 'logprobs', 'distribution')
            self.replay_buffer.add(state, action, reward, state_next, done, output['logprob'], output['entropy'])

            if done:
                # Reinitialise the state
                transitions = self.replay_buffer.get_list(self.device)
                loss, entropy = self.train_step(transitions)                    
...
#                     self.target_network.load_state_dict(self.network.state_dict())
    ...
    def train_step(self, transitions):
        #('state', 'action', 'reward', 'state_next', 'done', 'logprobs', 'distribution')
        states, actions, rewards, states_next, dones, logprobs, entropy = transitions
        #process rewards
        #
        new_rewards = []
        r_rewards = torch.flip(rewards, (0,1)).squeeze(-1).cpu().tolist()
        
        r_log_probs = torch.flip(logprobs, (0,)).squeeze(-1).cpu().tolist()
        r_entropies = torch.flip(entropy, (0,)).squeeze(-1).cpu().tolist()
        temp_reward = 0
        loss = 0
        temp_reward = torch.zeros(1, 1)
        for r,lp,ep in zip(r_rewards, r_log_probs, r_entropies):
            r1 = r
            if isinstance(r,list):
                r1 = r[0]
            temp_reward = r1 + (self.gamma * temp_reward)
            # loss = loss - (log_probs[i]*(Variable(R).expand_as(log_probs[i])).cuda()).sum() - (0.0001*entropies[i].cuda()).sum()
            loss = loss - (lp*torch.autograd.Variable(temp_reward, requires_grad=True).to(self.device) - 0.001*ep)
        
        torch.tensor(loss).to(self.device)
        # Update weights
...
#             return {"entropy": distribution.entropy(), "values": value, "actions":actions, "logprob":logprob}
#         else:
#             #TO CHANGE LATER
#             assert False
# #             if qs.dim() == 1:
